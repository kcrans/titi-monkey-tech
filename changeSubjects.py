...
def new_dictionary(list_values):
    ...
    new_dict_b["neg_response_time"] = next(list_values)
    new_dict["phase 1-3 params"] = new_dict_b
    new_dict_c = {}
    new_dict_c["negative_reinforcement_delay"] = next(list_values)
    new_dict_c["positive_reinforcement_delay"] = next(list_values)
    new_dict_c["hold_phase_delay"] = next(list_values)
    new_dict_c["circle_diam"] = next(list_values)
    new_dict_c["neg_response_time"] = next(list_values)
...
if subDlg.OK:
    ...
    if subject_choice == "NEW Subject":
        ...
        parDlg.addField("Negative response time:", "2.0")
        parDlg.addText("Parameters for phase 4:", color="Blue")    
        parDlg.addField("Negative reinforcement delay:", "3.0")
        parDlg.addField("Positive reinforcement delay:", "1.0")
        parDlg.addField("Hold phase delay:", "2")
        parDlg.addField("Circle diameter:", "0.8")
        ...
        parDlg.addField("Neg response time:", current_phase_b["neg_response_time"])
        parDlg.addText("Parameters for phase 4:", color="Blue")    
        current_phase_c = current_sub["phase 4 params"]
        parDlg.addField("Negative reinforcement delay:", current_phase_c["negative_reinforcement_delay"])
        parDlg.addField("Positive reinforcement delay:", current_phase_c["positive_reinforcement_delay"])
        parDlg.addField("Hold phase delay:", current_phase_c["hold_phase_delay"])
        parDlg.addField("Circle diameter:", current_phase_c["circle_diam"])
        parDlg.addField("Negative response time:", current_phase_c["neg_response_time"])
