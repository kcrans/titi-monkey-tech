...
def get_shape(mywin, shape_name):
    ...
    #create triangle stimuli
        ...
        square = visual.rect.Rect(
            win=mywin, size = (circle_diam - 0.1, circle_diam - 0.1), pos=(0,0),
        ...
        star = visual.ShapeStim(
            win=mywin, name='go_cross',
            size=(0.8, 0.8), vertices=star_points,
            ori=0.0, pos=(0, 0), anchor='center',
            lineWidth=0.0,     colorSpace='rgb', fillColor='grey',
            opacity=None, interpolate=True)
...
while 'escape' not in keys:
    ...
    tiny_circle1.pos = (side*(hor_pos), 0.5*0.8)
    tiny_circle2.pos = (side*(hor_pos) - 0.5*0.8, -0.5*0.8)
    tiny_circle3.pos = (side*(hor_pos) + 0.5*0.8,  -0.5*0.8)
    dis_shape.draw()
    tiny_circle1.draw()
    tiny_circle2.draw()
    tiny_circle3.draw()
